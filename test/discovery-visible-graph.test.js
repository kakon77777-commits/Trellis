...
async function community(store, id, discoverability) {
...
async function setup() {
  ...
  for (const id of ['actor:A', 'actor:B', 'actor:X', 'actor:Y']) (await register(store, id));
  (await community(store, 'community:C1', 'public'));
  (await community(store, 'community:Cprivate', 'private'));
...
test('discovery snapshot contains viewer-visible graph only', async () => {
...
test('adding a hidden fact does not change visible discovery snapshot hash', async () => {
  const { db, store } = (await setup());
  const before = (await buildDiscoverySnapshot({
    subjectActorId: 'actor:A',
    viewerContext: { viewer_actor_id: 'actor:A' },
    db,
    eventStore: store
  ...
  (await follow(store, 'actor:B', 'actor:Y', 'by-hidden', 'participants'));
  (await rebuildRelationshipProjection(db, store));

  const after = (await buildDiscoverySnapshot({
    subjectActorId: 'actor:A',
    viewerContext: { viewer_actor_id: 'actor:A' },
    db,
    eventStore: store
