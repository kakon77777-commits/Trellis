...
test('http presentation adapters and web have no storage or independent Authority imports',async ()=>{
...
test('http and web are intentionally absent from Foundation state-domain registry',async ()=>{
  assert.equal(CONTRACT_REGISTRY.http,undefined); assert.equal(CONTRACT_REGISTRY.web,undefined);
});

test('http server is only a composition root and contains no social queries or Authority decisions',async ()=>{
