...
async function join(db, store, actorId, communityId, suffix) {
  const r=(await requestMembership({command_id:`join:${suffix}`,idempotency_key:`join:${suffix}`,principal_id:`principal:${actorId}`,actor_id:actorId,community_id:communityId},ctx(db,store,actorId)));
...
async function follow(db, store, source, target, suffix) {
  return (await proposeRelationship({command_id:`follow:${suffix}`,idempotency_key:`follow:${suffix}`,principal_id:`principal:${source}`,source_entity_id:source,target_entity_id:target,relationship_type:'follows'},ctx(db,store,source))).relationship_id;
}
async function collab(db, store, source, target, suffix, visibility='public') {
  const r=(await proposeRelationship({command_id:`collab:${suffix}`,idempotency_key:`collab:${suffix}`,principal_id:`principal:${source}`,source_entity_id:source,target_entity_id:target,relationship_type:'collaborates_with',visibility},ctx(db,store,source)));
  (await activateRelationship({command_id:`activate:${suffix}`,idempotency_key:`activate:${suffix}`,principal_id:`principal:${target}`,relationship_id:r.relationship_id,expected_version:1},ctx(db,store,target)));
  return r.relationship_id;
}
...
test('Home Feed combines only visible explicit-source Publication roots', async () => {
  ...
  const feed=(await buildHomeFeed({subjectActorId:'actor:A',viewerContext:{viewer_actor_id:'actor:A'},db,eventStore:store,disclosurePolicy}));
...
test('viewer-invisible facts do not change Home Feed items ordering or snapshot', async () => {
  const {db,store,followX}=(await setup());
  const hidden=new Set([followX]);
  const disclosurePolicy=value=>value.relationship_id&&hidden.has(value.relationship_id)?'deny':'allow';
  ...
  const memberC2=(await join(db,store,'actor:A','community:C2','a-c2-hidden')); hidden.add(memberC2);
  const hiddenCollab=(await collab(db,store,'actor:B','actor:Y','b-y-hidden')); hidden.add(hiddenCollab);
  (await rebuildRelationshipProjection(db,store));
