...
function ctx(db,store,actor,extra={}){return {db,sql:db,eventStore:store,principalActorId:actor,capabilityGrants:[],evaluatedAt:'2026-09-02T14:00:00Z',...extra};}
...
async function setupMember(db,store,a,c){
  (await reg(store,a)); (await createCommunity({command_id:`cmd:create-${c}`,idempotency_key:`idem:create-${c}`,principal_id:`principal:${c}`,community_id:c},{eventStore:store,authorize:evaluateAuthority}));
  ...
  (await rebuildRelationshipProjection(db,store)); return r.relationship_id;
}

test('X1-X3 contract registry declares state class and effective inheritance for current domains', async () => {
...
test('X1 canonical visibility is a ceiling: policy narrows public but cannot widen private', async () => {
  ...
  const db=createTestDatabase(); const store=new SQLiteEventStore(db); (await reg(store,'actor:A')); (await reg(store,'actor:B'));
  (await createPublication((await pub('x1-public','actor:A')),ctx(db,store,'actor:A')));
  (await createPublication((await pub('x1-private','actor:A',{visibility:'private'})),ctx(db,store,'actor:A')));
  (await rebuildPublicationProjection(db,store));
  assert.equal((await loadPublicationSurface({publicationId:'pub:x1-public',viewerContext:{viewer_actor_id:'actor:B'},db,eventStore:store,disclosurePolicy:()=> 'deny'})),null);
  assert.equal((await loadPublicationSurface({publicationId:'pub:x1-private',viewerContext:{viewer_actor_id:'actor:B'},db,eventStore:store,disclosurePolicy:()=> 'allow'})),null);
  assert.equal(store.readStream('publication','pub:x1-private')[0].payload.visibility,'private');
});

test('X2 social membership alone cannot authorize Community Publication', async () => {
  const { createPublication } = require('../publication/service');
  const db=createTestDatabase(); const store=new SQLiteEventStore(db); (await setupMember(db,store,'actor:A','community:C'));
  const command=(await pub('x2','actor:A',{scope_ref:'community:C',visibility:'scope_members'}));
  ...
  (await createPublication(command,ctx(db,store,'actor:A',{capabilityGrants:[grant]})));
  const after=db.prepare("SELECT COUNT(*) AS n FROM canonical_events WHERE stream_type='relationship'").get().n;
  assert.equal(after,before);
});

test('X3 viewer-invisible private reply cannot change viewer-visible parent surface', async () => {
  const { createPublication } = require('../publication/service');
  ...
  (await createPublication((await pub('x3-parent','actor:A')),ctx(db,store,'actor:A')));
  (await rebuildPublicationProjection(db,store));
  const before=(await loadPublicationSurface({publicationId:'pub:x3-parent',viewerContext:{viewer_actor_id:'actor:B'},db,eventStore:store}));
  (await createPublication((await pub('x3-hidden','actor:C',{visibility:'private',reply_to_ref:'pub:x3-parent'})),ctx(db,store,'actor:C')));
  (await projectPublicationStream(db,store,'pub:x3-hidden'));
  const after=(await loadPublicationSurface({publicationId:'pub:x3-parent',viewerContext:{viewer_actor_id:'actor:B'},db,eventStore:store}));
