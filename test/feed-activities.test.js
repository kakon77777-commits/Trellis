...
async function follow(store, source, target, suffix) {
...
async function collaborate(store, source, target, suffix, extra = {}) {
  const pending = (await proposeRelationship({
...
async function setup() {
  ...
  (await join(store, 'actor:A', 'community:C', 'a-c'));
  const membershipB = (await join(store, 'actor:B', 'community:C', 'b-c'));
  (await follow(store, 'actor:A', 'actor:B', 'a-b'));
  const collabAB = (await collaborate(store, 'actor:B', 'actor:X', 'b-x', { visibility: 'public' }));
  const collabHidden = (await collaborate(store, 'actor:B', 'actor:Y', 'b-y', { visibility: 'public' }));
  (await rebuildRelationshipProjection(db, store));
  return { db, store, membershipB, collabAB, collabHidden };
}

test('Feed activity projector allowlists only membership and collaboration activation', async () => {
  ...
  const items = (await collectHomeActivityItems({
    sourceGraph,
    subjectActorId: 'actor:A',
    viewerContext: { viewer_actor_id: 'actor:A' },
    db,
...
test('termination contestation and evidence never create Feed activity items', async () => {
  const { db, store, collabAB } = (await setup());
  ...
  const newCollab = (await collaborate(store, 'actor:A', 'actor:B', 'a-b-2'));
...
test('current disclosure policy hides activity source before item projection', async () => {
  const { db, store, membershipB, collabHidden } = (await setup());
