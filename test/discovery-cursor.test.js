...
test('cursor pagination is stable and has no duplicates', async () => {
...
test('same state produces byte-stable cursor', async () => {
...
test('cursor rejects algorithm mismatch', async () => {
...
test('cursor rejects visible snapshot mismatch', async () => {
  const cursor=encodeCursor({ algorithm_ref:'algo:v1', snapshot_ref:'snap:1', last_score:8, last_entity_id:'actor:B' });
...
test('same snapshot ref keeps cursor valid even if unrelated hidden state changed elsewhere', async () => {
