...
test('target endpoint sees activate hint on bilateral pending relationship', async () => {
  const { availableRelationshipActions } = require('../relationship-surface/action-hints');
  const fx = (await pendingFixture());
...
test('source and unrelated viewer do not receive bilateral activate hint', async () => {
  const { availableRelationshipActions } = require('../relationship-surface/action-hints');
  const fx = (await pendingFixture());
  const state = foldRelationship(fx.store.readStream('relationship', fx.relationshipId));
...
test('terminated relationship never advertises activate', async () => {
  const { availableRelationshipActions } = require('../relationship-surface/action-hints');
  const state = {
...
test('stale activate hint does not bypass canonical state or authority checks', async () => {
  ...
  const first = await surface.activate(command, contextFor(fx.store, 'actor:B'));
  const retry = await surface.activate(command, contextFor(fx.store, 'actor:B'));
  assert.equal(retry.receipt.deduplicated, true);
  assert.equal(first.relationship_id, retry.relationship_id);

  await assert.rejects(async () => surface.activate({
    ...command,
...
test('social delegates_to relationship cannot authorize protected execution', async () => {
...
test('product command surface exposes only existing relationship service adapters', async () => {
  const surface = require('../relationship-surface/product-commands');
