...
async function register(store, actorId, key) {
...
async function buildVisibleFixture() {
  const db = createTestDatabase();
  const store = new SQLiteEventStore(db);
  (await register(store, 'actor:<A>', 'A'));
  (await register(store, 'actor:B', 'B'));
  const rel = (await proposeRelationship({
...
test('HTML and JSON render the same viewer-filtered relationship facts', async () => {
  ...
  const fx = (await buildVisibleFixture());
...
test('unreadable private relationship produces no relationship page projection or hidden evidence signal', async () => {
  const { loadRelationshipDetail } = require('../relationship-surface/read-service');
  const { renderRelationshipJson } = require('../relationship-surface/render-json');
  const { renderRelationshipHtml } = require('../relationship-surface/render-html');
  const db = createTestDatabase();
...
test('actor profile relationship preview links to relationship detail without absorbing history', async () => {
  const fx = (await buildVisibleFixture());
  const profile = (await buildActorProfile({ actorId: 'actor:<A>', eventStore: fx.store, db: fx.db }));
...
test('relationship renderers are pure presentation modules with no database or EventStore import', async () => {
