...
test('mobile page-level Context Lens is a collapsed disclosure after page lead and before page body on all five public surfaces', async () => {
  ...
  for (const path of ['/','/discover','/actors/actor%3AA','/publications/pub%3Ap1','/communities/community%3AC']) {
    ...
    assert.ok(lead >= 0);
    assert.ok(drawer > lead);
    assert.ok(body > drawer);
    assert.match(html, /<details class="mobile-context-drawer">\s*<summary>Context Lens<\/summary>/);
    assert.doesNotMatch(html, /<details class="mobile-context-drawer"[^>]*\sopen(?:\s|=|>)/);
  }
});
...
Mobile <=720px: single content column + bottom nav + default-collapsed Context Lens directly after page lead and before page body
```

On mobile, the persistent desktop Context Lens aside is hidden and the page-level Lens is rendered as a native `<details>` disclosure with no `open` attribute between `data-page-lead` and `data-page-body`. Semantic-edge visual layout becomes a one-column/adjacency presentation; do not scale a desktop graph below readable width.
