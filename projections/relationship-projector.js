...
function upsertRelationshipStatement(state) {
  return {
    sql: `
...
async function projectRelationshipStream(sql, eventStore, streamId) {
  const events = await eventStore.readStream('relationship', streamId);
  if (events.length === 0) return null;
  const state = foldRelationship(events);
  await sql.batch([upsertRelationshipStatement(state)]);
  return state;
}

async function projectRelationshipEvent(sql, eventStore, event) {
  if (event.stream_type !== 'relationship') return null;
  return await projectRelationshipStream(sql, eventStore, event.stream_id);
}

async function rebuildRelationshipProjection(sql, eventStore) {
  const streams = await sql.all(`
    SELECT DISTINCT stream_id
    FROM canonical_events
    WHERE stream_type = 'relationship'
    ORDER BY stream_id
  `);
  const statements = [{ sql: 'DELETE FROM relationships_current', params: [] }];
  for (const row of streams) {
    const events = await eventStore.readStream('relationship', row.stream_id);
    if (events.length === 0) continue;
    statements.push(upsertRelationshipStatement(foldRelationship(events)));
  }
