...
async function idempotencyGate(command,context,preferenceId){const commandDigest=digest(command);const prior=await context.eventStore.lookupIdempotency(command.idempotency_key);if(!prior)return{commandDigest,result:null};if(prior.command_digest!==commandDigest)throw new IdempotencyConflictError();return{commandDigest,result:{preference_id:preferenceId,receipt:{...prior,deduplicated:true}}};}
async function requireActiveActor(eventStore,actorId,code='PREFERENCE_OWNER_NOT_ACTIVE'){const events=await eventStore.readStream('entity',actorId);const state=events.length?foldEntity(events):null;if(!state||state.lifecycle!=='active'||state.entity_kind!=='actor'||state.actor_capable!==true) throw new InvalidTransitionError(code);return state;}
async function publicationTarget(targetRef,ownerActorId,context,{rootOnly=false}={}){
  const events=await context.eventStore.readStream('publication',targetRef);if(!events.length) throw new InvalidTransitionError('PREFERENCE_TARGET_NOT_FOUND');
...
async function socialActivityTarget(eventId,ownerActorId,context){
  const event=await context.eventStore.readEvent(eventId);if(!event||event.stream_type!=='relationship'||event.event_type!=='relationship.activated') throw new InvalidTransitionError('PREFERENCE_FEED_ACTIVITY_INVALID');
  const events=await context.eventStore.readStream('relationship',event.stream_id);if(!events.length) throw new InvalidTransitionError('PREFERENCE_FEED_ACTIVITY_INVALID');
...
async function validateTarget(type,normalized,ownerActorId,context){
  if(type==='bookmark_publication'||type==='not_interested_publication') return await publicationTarget(normalized.target_ref,ownerActorId,context);
  if(type==='mute_actor') return await requireActiveActor(context.eventStore,normalized.target_ref,'PREFERENCE_TARGET_ACTOR_NOT_ACTIVE');
  if(type==='dismiss_feed_item'&&normalized.target_item_kind==='publication') return await publicationTarget(normalized.target_ref,ownerActorId,context,{rootOnly:true});
  if(type==='dismiss_feed_item'&&normalized.target_item_kind==='social_activity') return await socialActivityTarget(normalized.target_ref,ownerActorId,context);
...
async function append(command,context,preferenceId,expectedVersion,eventType,suffix,payload,authorityReceipt,commandDigest){const timestamp=command.occurred_at??context.evaluatedAt??new Date().toISOString();const receipt=await context.eventStore.append({streamType:'preference',streamId:preferenceId,expectedVersion,events:[baseEvent(command,context,eventType,suffix,payload)],authorityReceipt,commandReceipt:{command_id:command.command_id,idempotency_key:command.idempotency_key,command_digest:commandDigest,status:'accepted',created_at:timestamp}});await projectPreferenceStream(context.sql,context.eventStore,preferenceId);return{preference_id:preferenceId,receipt};}
async function createPreference(command,context){
  ...
  const gate=await idempotencyGate(command,context,preferenceId);if(gate.result)return gate.result;
  await requireActiveActor(context.eventStore,command.owner_actor_id);
  if((await context.eventStore.readStream('preference',preferenceId)).length) throw new InvalidTransitionError('PREFERENCE_ALREADY_EXISTS');
  await validateTarget(command.preference_type,normalized,command.owner_actor_id,context);
  ...
  return await append(command,context,preferenceId,0,'preference.created','created',payload,authorityReceipt,gate.commandDigest);
}
async function loadForMutation(command,context){const events=await context.eventStore.readStream('preference',command.preference_id);if(!events.length) throw new InvalidTransitionError('PREFERENCE_NOT_FOUND');const state=foldPreference(events);if(state.owner_actor_id!==command.owner_actor_id) throw new InvalidTransitionError('PREFERENCE_OWNER_MISMATCH');return{events,state};}
async function withdrawPreference(command,context){
  requireMutationCommand(command);const gate=await idempotencyGate(command,context,command.preference_id);if(gate.result)return gate.result;const{state}=await loadForMutation(command,context);if(state.lifecycle!=='active') throw new InvalidTransitionError('PREFERENCE_CANNOT_WITHDRAW');
  const authorityReceipt=authorityOrThrow(authorityRequest(command,context,state.preference_id,'preference.withdraw',state));return await append(command,context,state.preference_id,command.expected_version,'preference.withdrawn','withdrawn',{reason:command.reason??'owner_withdrawn'},authorityReceipt,gate.commandDigest);
}
async function restorePreference(command,context){
  requireMutationCommand(command);const gate=await idempotencyGate(command,context,command.preference_id);if(gate.result)return gate.result;const{state}=await loadForMutation(command,context);if(state.lifecycle!=='withdrawn') throw new InvalidTransitionError('PREFERENCE_CANNOT_RESTORE');
  await requireActiveActor(context.eventStore,state.owner_actor_id);await validateTarget(state.preference_type,{target_kind:state.target_kind,target_ref:state.target_ref,target_item_kind:state.target_item_kind},state.owner_actor_id,context);
  const authorityReceipt=authorityOrThrow(authorityRequest(command,context,state.preference_id,'preference.restore',state));return await append(command,context,state.preference_id,command.expected_version,'preference.restored','restored',{},authorityReceipt,gate.commandDigest);
}
