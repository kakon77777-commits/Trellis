...
async function loadCurrentNotificationItem({row,viewerContext={},db,eventStore,disclosurePolicy}){
  ...
  const source=await resolveCurrentNotificationContext(normalized,{db,eventStore,disclosurePolicy});
...
async function buildNotificationInbox({recipientActorId,viewerContext={},db,eventStore,disclosurePolicy}){
  assertInboxReadAuthorized(recipientActorId,viewerContext);
  const rows=await db.all(`SELECT * FROM notifications_current WHERE recipient_actor_id=? ORDER BY issued_global_offset DESC, notification_id DESC`,[recipientActorId]);
  const items=[];
  for(const row of rows){
    const item=await loadCurrentNotificationItem({row,viewerContext,db,eventStore,disclosurePolicy});
    if(item) items.push(item);
  }
  const preferredItems=await applyOwnerNotificationPreferences({ownerActorId:recipientActorId,viewerContext,items,db});
...
async function loadNotificationInboxSurface({recipientActorId,viewerContext={},db,eventStore,disclosurePolicy,limit=20,cursor=null}){
  ...
  const base=await buildNotificationInbox({recipientActorId,viewerContext,db,eventStore,disclosurePolicy});
