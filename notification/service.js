...
const {resolveCurrentNotificationContextAsync}=require('./read-policy');
...
async function processSourceEvent(input,context){
  requireInput(input);
  const sourceEvent=await context.eventStore.readEvent(input.eventId);
  if(!sourceEvent) throw new InvalidTransitionError('NOTIFICATION_SOURCE_EVENT_NOT_FOUND');
  const derived=await deriveNotificationCandidate(sourceEvent,context);
  ...
  const existing=await context.eventStore.readStream('notification',notificationId);
  ...
  await projectNotificationStream(context.sql,context.eventStore,notificationId);
...
async function acknowledgeNotification(command,context){
  ...
  const events=await context.eventStore.readStream('notification',command.notification_id);
  ...
  const source=await resolveCurrentNotificationContextAsync(state,{db:context.db,eventStore:context.eventStore,disclosurePolicy:context.disclosurePolicy});
  ...
  await projectNotificationStream(context.sql,context.eventStore,state.notification_id);
