...
function createResourceRoutes(){
  return async ({url,services})=>{
    ...
    if(id!==null){const value=await services.loadActor(id); return value?{status:200,headers:{'content-type':'text/html; charset=utf-8'},body:renderProfilePage(value)}:notFound();}
    id=decodedMatch(url.pathname,/^\/publications\/([^/]+)$/);
    if(id!==null){const value=await services.loadPublication(id); return value?{status:200,headers:{'content-type':'text/html; charset=utf-8'},body:renderPublicationPage(value)}:notFound();}
    id=decodedMatch(url.pathname,/^\/communities\/([^/]+)$/);
    if(id!==null){const value=await services.loadCommunity(id); return value?{status:200,headers:{'content-type':'text/html; charset=utf-8'},body:renderCommunityPage(value)}:notFound();}
    id=decodedMatch(url.pathname,/^\/api\/actors\/([^/]+)$/);
    if(id!==null){const value=await services.loadActor(id); return value?jsonResponse(value):notFound();}
    id=decodedMatch(url.pathname,/^\/api\/publications\/([^/]+)$/);
    if(id!==null){const value=await services.loadPublication(id); return value?jsonResponse(value):notFound();}
    id=decodedMatch(url.pathname,/^\/api\/communities\/([^/]+)$/);
    if(id!==null){const value=await services.loadCommunity(id); return value?jsonResponse(value):notFound();}
    return null;
  };
}
