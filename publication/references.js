...
async function actorCanReadParentAtCreation(parent,actorId,db){
  ...
  switch(parent.visibility){
    ...
    case'scope_members':return await isActiveCommunityMember(db,parent.scope_ref,actorId);
    default:return false;
  }
}
function effectiveParticipants(publication){return new Set([publication.author_actor_id,...(publication.audience_actor_ids??[])]);}
function allIn(set,allowed){for(const value of set)if(!allowed.has(value))return false;return true;}

async function isChildAudienceSubsetOfParent({childDraft,parentState,db}){
  if(parentState.visibility==='public')return true;
  if(parentState.visibility==='private')return childDraft.visibility==='private';
  ...
  if(parentState.visibility==='scope_members'){
    ...
    if(childDraft.visibility==='participants'){
      ...
      return [...effectiveParticipants(childDraft)].every(actorId=>resolver(parentState.scope_ref,actorId));
    }
    return false;
  }
  return false;
...
async function validatePublicationReferenceCreation({childDraft,parentState,actorId,db}){
  ...
  if(!(await actorCanReadParentAtCreation(parentState,actorId,db)))invalid('PUBLICATION_REFERENCE_NOT_READABLE');
  if(!(await isChildAudienceSubsetOfParent({childDraft,parentState,db})))invalid('PUBLICATION_REFERENCE_AUDIENCE_WIDENED');
  return true;
}

async function resolveReferenceContext({publication,viewerContext={},db,disclosurePolicy,membershipResolver}){
  ...
  const row=await db.first('SELECT * FROM publications_current WHERE publication_id = ?',[targetRef]);
  ...
  return{status:'active',publication_id:target.publication_id,author_actor_id:target.author_actor_id,preview:target.current_body.slice(0,160)};
}

module.exports={validatePublicationReferenceCreation,isChildAudienceSubsetOfParent,actorCanReadParentAtCreation,resolveReferenceContext};
